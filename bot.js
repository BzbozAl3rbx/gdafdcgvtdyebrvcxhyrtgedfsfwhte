...
client.user.setGame(`Chloè.`,"http://twitch.tv/S-F")
  console.log('')
  console.log('')
  console.log('╔[═════════════════════════════════════════════════════════════════]╗')
...
client.on('message', message => {
  ...
  if(!message.member.hasPermission('ADMINISTRATOR')) return      message.channel.send('**للأسف لا تمتلك صلاحية** `ADMINISTRATOR`' );
    ...
    if (!args) return message.reply('**الرجاء كتابة كلمة او جملة لإرسال البرودكاست**');message.channel.send(`**هل أنت متأكد من إرسالك البرودكاست؟ \nمحتوى البرودكاست:** \` ${args}\``).then(msg => {
    msg.react('✅')
    .then(() => msg.react('❌'))
    .then(() =>msg.react('✅'))
...
client.on('message', message => {
    ...
    if (message.author.id !== '406348958139678720') return message.reply('** هذا الأمر فقط لصاحب البوت و شكراًً **')
...
client.on('message', message => {
  ...
  message.channel.send(`** .تم إرسال الرابط على الخاص **`)
...
var args = message.content.split(" ").slice(1);
    ...
    if(!tomute) return message.reply("**الرجاء منشن شخص** ") .then(m => m.delete(5000));
...
setTimeout(function(){
  ...
  if(!message.member.hasPermission("MANAGE_ROLES")) return message.channel.sendMessage("**ليس لديك صلاحية لفك عن هذا الشخص الميوت** ").then(m => m.delete(5000));
if(!message.guild.member(client.user).hasPermission("MANAGE_ROLES")) return message.reply("**I Don't Have `MANAGE_ROLES` Permission**").then(msg => msg.delete(6000))

  let toMute = message.guild.member(message.mentions.users.first()) || message.guild.members.get(args[0]);
  if(!toMute) return message.channel.sendMessage("**الرجاء تحديد شخص**");

  let role = message.guild.roles.find (r => r.name === "muted");
  
  if(!role || !toMute.roles.has(role.id)) return message.channel.sendMessage("**لم يتم اعطاء هذا الشخص ميوت من الأساس**")

  await toMute.removeRole(role)
  message.channel.sendMessage("**لقد تم فك الميوت عن الشخص بنجاح**");
...
client.on("message", message => {
 ...
 message.channel.send("**تم إرسال الاوامر على الخاص**")
  const embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setThumbnail(message.author.avatarURL)
      .setDescription(`
**اوامر بوت Chloè**
● الاوامر العامه ●
-  「!id 」  「 لمعرفة عدد الدعوات الخاصة بك في السيرفر 」
-    「 !avatar 」  「 لظهار الصورة الخاصه بـ العضو 」
-    「 !invite 」  「 ليرسل لك البوت رابط في الخاص 」
...
var kboosh = new Eris("NDYzNzA5NDcxODMxNjIxNjMz.Dh77mA.7wrRUoWbhf6wG-OxjR-2p3qet5E");
var kboosh_id = "464212538876493824";
                    var i = "0";
                    var x = "0";
kboosh.on("voiceChannelJoin", (msg) => {
    x++;
    kboosh.editChannel(kboosh_id, { name : ".Chloè「" + x + "」"});
});
...
kboosh.on("messageCreate", (msg) => {
    if(msg.author.id !== "406348958139678720") return kboosh.createMessage('__**This Command is only for the bot Owner**__');
...
kboosh.connect("NDYzNzA5NDcxODMxNjIxNjMz.Dh77mA.7wrRUoWbhf6wG-OxjR-2p3qet5E")
